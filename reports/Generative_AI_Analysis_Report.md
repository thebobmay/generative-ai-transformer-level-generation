...
**Difficulty misclassification.** The failure probe generated 90 samples (30 per class) and surfaced five misclassifications (5.6%), consistent with the difficulty match accuracy reported in evaluation. All five were Medium-conditioned outputs, which is consistent with Medium being the boundary class on both sides. Two scored Easy, with scores of 0.31 and 0.85, because their ground rows were essentially solid and their few enemies and blocks did not lift the score past the Easy threshold of 5.68. One scored Hard at 30.07, above the Hard threshold of 20.78, because its ground row carried enough of a gap. This illustrates threshold boundary drift, where an output whose computed score lands near a percentile threshold is assigned to an adjacent class regardless of the conditioning token. The underlying cause is structural: the model has not learned a semantic notion of difficulty, only structural associations with control tokens that are mediated entirely by the scoring formula, so any output whose score crosses a threshold is relabelled.
