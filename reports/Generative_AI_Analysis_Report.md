...
The longest ground gap carries the dominant weight because pit navigation is the primary mechanical challenge in Super Mario Bros gameplay. The choice of structural features such as enemy density, gaps, and hazards as difficulty signals follows established practice in the procedural content generation via machine learning literature (Summerville et al., 2018). The feature weights are heuristic and exploratory rather than empirically validated. This reflects a broader reality in the field: there is no consensus methodology for evaluating procedural level generation systems, and difficulty in particular lacks a standard quantitative definition (Withington et al., 2024). The scoring formula here is therefore presented as one reasonable heuristic among many rather than a ground-truth measure of difficulty.
...
**Difficulty match accuracy** was 94.0% overall for the conditional model on the 15-level corpus, with per-class F1 scores of 0.933 (Easy), 0.915 (Medium), and 0.970 (Hard), and a macro F1 of 0.940. These results are well above the 33% chance baseline and confirm that the difficulty control token has a measurable effect on generated output structure. Medium achieved the lowest F1 because it occupies the interval between both score thresholds. Its precision was high (0.98) but its recall lower (0.86), meaning Medium conditioned outputs often drift into an adjacent class even though anything scored Medium is almost always correct.

Expanding the corpus to 35 levels improved overall accuracy to 95.3% and macro F1 to 0.953, with per-class F1 rising for Easy (0.962) and Medium (0.926) and holding for Hard (0.970). The improvement reflects the additional structural variety within each difficulty class provided by the Lost Levels data, which gives the model more diverse examples of what each control token should produce.

**Diversity** was 78.0% for the conditional model on the 15-level corpus: 117 of 150 generated segments were distinct tile grids. Expanding the corpus improved diversity slightly to 79.3%.

**Nearest-neighbour similarity** revealed the extent of memorization. The mean maximum tile similarity between a generated segment and its closest training chunk was 0.990 for the conditional model on the 15-level corpus, meaning generated outputs differed from their nearest training match in approximately 4 to 5 tile positions out of 448. Expanding the corpus raised mean similarity slightly to 0.993 because the larger corpus covers more structural space, making it more likely that any generated output finds a close training match.

**Sampling experiments** showed that temperature is the most effective lever for reducing memorization behavior. At temperature 1.2, diversity reached 89.3% and mean max similarity dropped to 0.986 while difficulty match accuracy held at 97.3%, matching the temperature 1.0 setting. At temperature 1.5, diversity reached 98.0% and mean max similarity dropped to 0.957, but accuracy fell to 88.0%. The novelty rate, the fraction of outputs more than 10% different from any training chunk, rose from 0% at temperature 0.8 to 14.7% at temperature 1.5, the only configuration that produced a meaningful share of genuinely new structure. Temperature 1.2 represents the best practical trade-off for applications requiring varied output without severe accuracy degradation.

Top-k sampling produced modest improvements over the temperature 1.0 baseline with minimal accuracy impact, though no configuration meaningfully reduced mean max similarity. Nucleus sampling (top-p; Holtzman et al., 2020) was counterproductive for this vocabulary: at low p values the sky tile dominates the probability nucleus, removing structural tiles from the sampling distribution and collapsing diversity to 6% at p equal to 0.80. This is a direct consequence of the 86% sky tile frequency and is specific to heavily skewed tile distributions.

**A model capacity experiment** tested the central claim that memorization is driven by capacity exceeding the available data. Two smaller models were trained on the same 15-level corpus and identical hyperparameters and evaluated on the same metrics. The result is a clean monotonic curve. As capacity grows from 81 thousand to 458 thousand to 3.28 million parameters, difficulty match rises (0.380, 0.540, 0.940) and mean similarity rises with it (0.908, 0.930, 0.990), while diversity and novelty fall. Reducing capacity does reduce memorization, which confirms the mechanism, but the smallest model still reproduces about 91% of its nearest training chunk and only got there by collapsing difficulty match to the 0.333 chance floor. There is no smaller model that both conditions well and avoids memorizing. Together with the corpus expansion result, where more data did not escape memorization either, this points past model size to data scale, and specifically the availability of structurally diverse data beyond the Mario game family, as the binding constraint under these training conditions.
...
The program can be given a difficulty instruction before it starts generating. When told to make an Easy level, it tends to produce sections with solid, unbroken ground and few obstacles. When told Hard, it tends to produce sections with longer ground gaps that require bigger jumps. This difficulty control works about 94% of the time on the original training data and about 95% of the time after adding the Lost Levels, when measured against the same scoring system used to label the training data.
...
**Difficulty misclassification.** The failure probe generated 90 samples (30 per class) and surfaced five misclassifications (5.6%), consistent with the difficulty match accuracy reported in evaluation. Three were Medium conditioned. Two of those scored Easy, with scores of 0.31 and 0.85, because their ground rows were essentially solid and their few enemies and blocks did not lift the score past the Easy threshold of 5.68, and one scored Hard at 30.07, above the Hard threshold of 20.78, because its ground row carried enough of a gap. The other two were Hard conditioned and both fell short of the Hard threshold of 20.78, one scoring Easy at 5.47 and one scoring Medium at 20.16. No Easy conditioned output was misclassified. This illustrates threshold boundary drift, where an output whose computed score lands near a percentile threshold is assigned to an adjacent class regardless of the conditioning token. The underlying cause is structural: the model has not learned a semantic notion of difficulty, only structural associations with control tokens that are mediated entirely by the scoring formula, so any output whose score crosses a threshold is relabelled.

**Near-verbatim reproduction.** Two reproduction examples were found, both exact copies. An Easy-conditioned output matched a chunk from mario-6-2 column 0 at 1.0 similarity with zero differing tiles, and a Medium-conditioned output matched a chunk from mario-4-1 column 144, also a perfect tile-for-tile copy. This is consistent with the 0.990 mean nearest-neighbour similarity measured in the evaluation section. The model has memorized specific training sequences and retrieves them under the appropriate conditioning context rather than composing new arrangements.

**Pipe integrity violations.** Seven of 150 samples (4.7%) contained pipe top tokens (`<>`) without pipe body tokens (`[]`) directly below, producing structures that are tile-vocabulary-valid but semantically impossible to render correctly. These violations arise from the autoregressive sampling process: the model predicts a pipe top token at a row where the training distribution supports it, but subsequent row predictions choose other content rather than a pipe shaft. Because the structural validity check only verifies tile vocabulary and grid dimensions, these violations pass validation.

**Scoring-weight artifact in Easy samples.** Twelve of 50 Easy-scored outputs (24%) contained six or more enemy tiles. The worst example had 12 enemies with a solid ground row, scoring Easy because the longest gap was zero. This is a direct consequence of the heuristic scoring design: the gap feature dominates difficulty score by a factor of roughly 6:1 over enemy density at typical values. The model has learned that Easy conditioning correlates with solid ground rows and with certain structural patterns from the training data, some of which include high enemy counts.
...
**Deployment considerations.** The model is intended as a research prototype for exploring conditional generation. It should not be deployed in a production system without human review of generated outputs, validation against a broader set of difficulty criteria, and disclosure to end users that content is algorithmically generated. Automated suppression or promotion of generated level content based solely on classifier output without human oversight would be inappropriate. The memorization behavior identified in evaluation means that deployed outputs may closely replicate training data, raising additional questions about appropriate use in commercial contexts. Documenting the model's intended use, training data, and known limitations in the manner of a model card would support responsible deployment (Mitchell et al., 2019).

---

## Limitations and Future Improvements

**Corpus size and memorization.** The most significant limitation is memorization, and the capacity experiment shows it is governed by the ratio of model capacity to data rather than by model size alone. With 128 to 312 training sequences and a 3.28 million parameter model, the model has sufficient capacity to memorize training data rather than learn generalizable structural rules. Mean nearest-neighbour tile similarity of 0.99 at the default sampling temperature confirms this, and only aggressive high-temperature sampling reduces it appreciably. The experiment also shows the limit is not fixable by tuning the model: shrinking it lowers memorization only by sacrificing difficulty conditioning, and the corpus expansion showed that adding the available VGLC data does not escape it either. Both levers fail within the resources at hand, which points to data scale as the binding constraint. A masked loss approach, removing the sky tile from the training signal while retaining it as context, is a candidate near term experiment that requires no architectural changes and could redirect model capacity toward the 14% of tiles that define level structure.
