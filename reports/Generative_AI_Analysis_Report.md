...
The project trains a conditional conditional model and a baseline unconditional model, evaluates them on four metrics (structural validity, diversity, nearest-neighbour similarity, and difficulty match accuracy), and extends the analysis with a corpus expansion experiment and a sampling strategy experiment. Key results show strong difficulty alignment (96% baseline accuracy, 97% with expanded corpus) but near-verbatim reproduction of training data in most outputs, with mean nearest-neighbour tile similarity above 0.99.
...
**Difficulty match accuracy** was 96.0% overall for the baseline model, with per-class F1 scores of 0.961 (Easy), 0.940 (Medium), and 0.980 (Hard), and a macro F1 of 0.960. These results are well above the 33% chance baseline and confirm that the difficulty control token has a measurable effect on generated output structure. Hard achieved the highest F1 because outputs conditioned on Hard reliably produce large ground gaps that push the difficulty score above the Hard threshold. Medium achieved the lowest F1 because it occupies the interval between both thresholds, and outputs near a threshold boundary can tip into either adjacent class with small changes in gap length or enemy placement.

Expanding the corpus to 35 levels improved overall accuracy to 96.7% and macro F1 to 0.966. The improvement reflects the additional structural variety within each difficulty class provided by the Lost Levels data, which gives the model more diverse examples of what each control token should produce.

**Diversity** was 76.0% for the baseline model: 114 of 150 generated segments were distinct tile grids. Expanding the corpus improved diversity to 83.3%, confirming that more training data reduces the model's tendency to collapse onto a small set of high-probability patterns.

**Nearest-neighbour similarity** revealed the extent of memorization. The mean maximum tile similarity between a generated segment and its closest training chunk was 0.990 for the baseline model, meaning generated outputs differed from their nearest training match in approximately 4 to 5 tile positions out of 448. Expanding the corpus raised mean similarity slightly to 0.993 because the larger corpus covers more structural space, making it more likely that any generated output finds a close training match.

**Sampling experiments** showed that temperature is the most effective lever for reducing memorization behavior. At temperature 1.2, diversity reached 90.7% and mean max similarity dropped to 0.985 while difficulty match accuracy remained at 93.3%. At temperature 1.5, diversity reached 98.0% and mean max similarity dropped to 0.944, but accuracy fell to 84.0%. Temperature 1.2 represents the best practical trade-off for applications requiring varied output without severe accuracy degradation.

Top-k sampling produced modest improvements over the temperature 1.0 baseline with minimal accuracy impact, though no configuration meaningfully reduced mean max similarity. Nucleus sampling (top-p) was counterproductive for this vocabulary: at low p values the sky tile dominates the probability nucleus, removing structural tiles from the sampling distribution and collapsing diversity to below 10%. This is a direct consequence of the 86% sky tile frequency and is specific to heavily skewed tile distributions.
...
**Difficulty misclassification.** Five of 90 generated samples (5.6%) scored at the wrong difficulty class, consistent with the 96% accuracy reported in evaluation. Two failure subtypes were observed. The first is threshold boundary drift: a Medium-conditioned output with a multi-gap ground row scored Hard (score 25.0, just above the Hard threshold of 20.78), and a Hard-conditioned output with only a small gap scored Medium (score 10.0). These near-threshold samples can tip across a class boundary with a single additional gap tile. The second is gap-feature collapse: a Hard-conditioned output with floating platforms and multiple enemies but a fully solid ground row scored Easy (score 0.78). Because the heuristic gives a weight of 5 points per gap tile but enemy density rarely contributes more than 0.9 points total, any output with no ground gap scores below the Easy threshold regardless of enemy count.

**Near-verbatim reproduction.** Two of 40 sampled outputs showed tile similarity of exactly 1.0 to a training chunk, confirmed as perfect reproductions of mario-8-1 column 96 and mario-4-1 column 112. This is consistent with the 0.990 mean nearest-neighbour similarity measured in the evaluation section. The model has memorized specific training sequences and retrieves them under the appropriate conditioning context. Notably, the mario-8-1 chunk contains 10 enemy tiles and still scores as Easy, illustrating that memorized Easy patterns can include high enemy density.

**Pipe integrity violations.** Two of 150 samples (1.3%) contained pipe top tokens (`<>`) without pipe body tokens (`[]`) directly below, producing structures that are tile-vocabulary-valid but semantically impossible to render correctly. These violations arise from the autoregressive sampling process: the model predicts a pipe top token at a row where the training distribution supports it, but subsequent row predictions choose ground continuation rather than a pipe shaft. Because the structural validity check only verifies tile vocabulary and grid dimensions, these violations pass validation.

**Scoring-weight artifact in Easy samples.** Thirteen of 50 Easy-scored outputs (26%) contained six or more enemy tiles. The worst example had 10 enemies with a solid ground row, scoring Easy because the longest gap was zero. This is a direct consequence of the heuristic scoring design: the gap feature dominates difficulty score by a factor of roughly 6:1 over enemy density at typical values. The model has learned that Easy conditioning correlates with solid ground rows and with certain structural patterns from the training data, some of which include high enemy counts.
